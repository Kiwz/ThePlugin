	...
	private boolean place(CommandSender sender, String[] args) {
		...
		for (String arg : args) {
			if (!arg.matches("[a-zA-Z-_0-9æøåÆØÅ]+")) {
				sendWarning(sender, "Tillatte tegn er a-å A-Å 0-9 - _");
				return true;
			}
		}
		...
		if (args.length == 0) {
			...
			if (args[0].equalsIgnoreCase("liste")) {
				Util.sendAsPages(sender, "1", 6, "Plass-Navn [Eier]", Place.getPlaceList());
			} else if (args[0].equalsIgnoreCase("spiller")) {
				Util.sendAsPages(sender, "1", 6, "Eier [Plass-Navn]", Place.getPlayerList());
			} else if (args[0].equalsIgnoreCase("her")) {
				if (player == null) {
					sendWarning(sender, "Consolen kan ikke bruke /plass " + args[0]);
				} else {
			...
			if (args[0].equalsIgnoreCase("liste")) {
				Util.sendAsPages(sender, args[1], 6, "Plass-Navn [Eier]", Place.getPlaceList());
			} else if (args[0].equalsIgnoreCase("spiller")) {
				if (args[1].matches("[0-9]+") && args[1].length() < 3) {
					Util.sendAsPages(sender, args[1], 6, "Eier [Plass-Navn]", Place.getPlayerList());
				} else {
					myPlayer = MyPlayer.getPlayer(args[1]);
					if (myPlayer == null) {
						sendWarning(sender, args[1] + " er ikke en spiller her");
					} else {
						...
						for (Place p : Place.getPlacesByOwner(target)) {
							ownerOf = ownerOf + "[" + Color.VARIABLE + p.getName() + Color.INFO + "] ";
						}
						...
						for (Place p : Place.getPlacesByMember(target)) {
							memberOf = memberOf + "[" + Color.VARIABLE + p.getName() + Color.INFO + "] ";
						}
				...
				if (player == null) {
					...
					if (newPlace.savePlace(player)) {
						sendInfo(sender, "Din nye plass heter " + newPlace.getName() + ", for info skriv: /plass " + newPlace.getName());
					} else {
				...
				if (player == null) {
					...
					if (place.savePlace(player)) {
						sendInfo(sender, place.getName() + " er flyttet hit");
					} else {
				...
				if (player == null) {
					...
					if (place.hasAccess(player) || (!place.getPriv() && Util.isSpawnSafe(place.getSpawn()))) {
						...
						sendWarning(sender, place.getName() + " har ingen spawn");
					}
				}
				...
				if (player == null) {
					...
					place.setSpawn(player.getLocation());
					if (place.savePlace(player)) {
						sendInfo(sender, place.getName() + " sin spawn er flyttet hit");
					} else {
						sendWarning(sender, place.getError(player));
					}
				}
				...
				if (place.deletePlace(player)) {
					sendInfo(sender, place.getName() + " er slettet");
				} else {
				...
				if (place.savePlace(player)) {
					sendInfo(sender, "Velkomstmelding er satt til standard for " + place.getName());
				} else {
					sendWarning(sender, place.getError(player));
				}
				...
				if (place.savePlace(player)) {
					sendInfo(sender, "Forlatemelding er satt til standard for " + place.getName());
				} else {
					sendWarning(sender, place.getError(player));
				}
			...
			if (args[0].equalsIgnoreCase("ny")) {
				...
				if (place.savePlace(player)) {
					sendInfo(sender, "Du har endret radiusen for " + place.getName() + " til " + place.getRadius());
				} else {
					sendWarning(sender, place.getError(player));
				}
			} else if (args[0].equalsIgnoreCase("entre")) {
				...
				if (place.savePlace(player)) {
					sendInfo(sender, "Ny velkomstmelding for " + place.getName() + " er: " + place.getEnter());
				} else {
					sendWarning(sender, place.getError(player));
				}
			} else if (args[0].equalsIgnoreCase("forlate")) {
				...
				if (place.savePlace(player)) {
					sendInfo(sender, "Ny forlatemelding for " + place.getName() + " er: " + place.getLeave());
				} else {
					sendWarning(sender, place.getError(player));
				}
			} else if (args[0].equalsIgnoreCase("navn")) {
				...
				if (place.savePlace(player)) {
					sendInfo(sender, oldName + " har byttet navn til " + place.getName());
				} else {
					sendWarning(sender, place.getError(player));
				}
			} else if (args[0].equalsIgnoreCase("toggle")) {
				if (args[2].equalsIgnoreCase("privat")) {
					if (place.getPriv()) {
						place.setPriv(false);
						if (place.savePlace(player)) {
							sendInfo(sender, place.getName() + " er nå offentlig");
						} else {
							sendWarning(sender, place.getError(player));
						}
					} else {
						place.setPriv(true);
						if (place.savePlace(player)) {
							sendInfo(sender, place.getName() + " er nå privat");
						} else {
							sendWarning(sender, place.getError(player));
						}
					}
					...
					if (place.getPvP()) {
						place.setPvP(false);
						if (place.savePlace(player)) {
							sendInfo(sender, place.getName() + " er ikke i PvP modus");
						} else {
							sendWarning(sender, place.getError(player));
						}
					} else {
						place.setPvP(true);
						if (place.savePlace(player)) {
							sendInfo(sender, place.getName() + " er nå i PvP modus");
						} else {
							sendWarning(sender, place.getError(player));
						}
					}
					...
					if (place.getMonsters()) {
						...
						if (place.savePlace(player)) {
							sendInfo(sender, "Monstre vil ikke spawne i " + place.getName());
						} else {
							sendWarning(sender, place.getError(player));
						}
					} else {
						...
						if (place.savePlace(player)) {
							sendInfo(sender, "Monstre vil nå spawne i " + place.getName());
						} else {
							sendWarning(sender, place.getError(player));
						}
					}
					...
					if (place.getAnimals()) {
						...
						if (place.savePlace(player)) {
							sendInfo(sender, "Dyr (og villager) vil ikke spawne i " + place.getName());
						} else {
							sendWarning(sender, place.getError(player));
						}
					} else {
						...
						if (place.savePlace(player)) {
							sendInfo(sender, "Dyr (og villager) vil nå spawne i " + place.getName());
						} else {
							sendWarning(sender, place.getError(player));
						}
					}
				...
				if (myPlayer == null) {
					sendWarning(sender, args[2] + " er ikke en spiller her");
				} else {
					Player target = myPlayer.getBukkitPlayer();
					if (place.setMember(target)) {
						if (place.savePlace(player)) {
							sendInfo(sender, target.getName() + " er nå medlem av " + place.getName());
						} else {
							sendWarning(sender, place.getError(player));
						}
					} else {
						if (place.savePlace(player)) {
							sendWarning(sender, target.getName() + " er allerede medlem av " + place.getName());
						} else {
							sendWarning(sender, place.getError(player));
						}
					}
				...
				if (myPlayer == null) {
					...
					if (place.removeMember(target)) {
						if (place.savePlace(player)) {
							sendInfo(sender, target.getName() + " er nå sparket ut av " + place.getName());
						} else {
							sendWarning(sender, place.getError(player));
						}
					} else {
						if (place.savePlace(player)) {
							sendWarning(sender, target.getName() + " er ikke medlem av " + place.getName());
						} else {
							sendWarning(sender, place.getError(player));
						}
					}
				...
				if (myPlayer == null) {
					...
					if (place.savePlace(player)) {
						sendInfo(sender, target.getName() + " er nå eier av " + place.getName());
						sendInfo(sender, "Du er dermed degradert til medlem av " + place.getName());
					} else {
						sendWarning(sender, place.getError(player));
					}
				}
